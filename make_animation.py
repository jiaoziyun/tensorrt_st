...
def make_animation(source_image, source_semantics, target_semantics,
    ...
    with torch.no_grad():
        ...
        for frame_idx in tqdm(range(target_semantics.shape[1]), 'Face Renderer:'):
            ...
            kp_norm = kp_driving
            out,_,_ = generator(source_image, kp_source=kp_source['value'], kp_driving=kp_norm['value'])
            '''
        ...
        tensor_predictions = [torch.from_numpy(pred) for pred in predictions]
        predictions_ts = torch.stack(tensor_predictions, dim=1)
